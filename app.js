(function () {
  ...
  function wireNoteHighlighting() {
    ...
    // Handle the tap directly on touchend: calling preventDefault() on touchstart would
    // suppress the browser's emulated click entirely, so touch devices would never fire it.
    els.addNoteBtn.addEventListener(
      "touchend",
      (event) => {
        event.preventDefault();
        handleAddNoteClick();
  ...
  function positionAddNoteBtn(rect, answerRect) {
    ...
    const btnHeight = els.addNoteBtn.offsetHeight;
    const btnHalfWidth = els.addNoteBtn.offsetWidth / 2;
    const viewportWidth = document.documentElement.clientWidth;
    const margin = 8;

    // Selecting the answer's first line leaves no room above it for the popup before it
    // overlaps the reveal button sitting right above the answer — flip below in that case.
    const placeBelow = rect.top - answerRect.top < btnHeight + margin;
    const top = placeBelow
      ? rect.bottom + window.scrollY + margin
      : rect.top + window.scrollY - btnHeight - margin;

    const rawLeft = rect.left + window.scrollX + rect.width / 2;
    const left = Math.min(
      Math.max(rawLeft, window.scrollX + btnHalfWidth + margin),
      window.scrollX + viewportWidth - btnHalfWidth - margin
    );

    els.addNoteBtn.style.top = `${Math.max(top, window.scrollY + margin)}px`;
